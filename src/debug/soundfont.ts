...
import { fetchSoundfont, startNote, startChord, isFetched } from "../soundfont.ts";
...
app.innerHTML = `
  <h1>SoundFont</h1>
  <p id="load-status" class="text-muted">Loading piano samples…</p>

  <section class="stack-sm">
    <h2>Keyboard</h2>
    <p class="text-muted">Press and hold keys to play piano samples.</p>
...
// ── Fetch soundfont script on page load (no user gesture needed) ────

const loadStatus = document.getElementById("load-status")!;

fetchSoundfont()
  .then(() => {
    loadStatus.textContent = "Samples loaded — click a key to start";
    loadStatus.className = "text-success";
  })
  .catch((err: unknown) => {
    loadStatus.textContent = `Failed to load: ${err}`;
    loadStatus.className = "text-error";
  });
